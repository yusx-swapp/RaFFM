...
class VisionTransformer_Orth(nn.Module):
    def __init__(self, model: VisionTransformer, blocks):
        ...
        self.cls_token, self.dist_token = copy.deepcopy(model.cls_token), copy.deepcopy(
            model.dist_token
        )
        self.pos_embed, self.pos_drop = copy.deepcopy(model.pos_embed), copy.deepcopy(
            model.pos_drop
        )
        self.blocks = blocks
        self.norm = copy.deepcopy(model.norm)
        self.pre_logits = copy.deepcopy(model.pre_logits)
        self.head, self.head_dist = copy.deepcopy(model.head), copy.deepcopy(
            model.head_dist
        )
...
class Linear_Orth(nn.Module):
    def __init__(self, fc: nn.Linear, keep: float):
        ...
        self.fc_V = nn.Linear(
            self.u_out_features, self.out_features, bias=self.has_bias
        )
        w = fc.weight.detach().data.t()
        U, s, V = torch.svd(w)
        Us, Vs = U * torch.sqrt(s), V * torch.sqrt(s)
        self.fc_U.weight.data.copy_(Us.t())
        self.fc_V.weight.data.copy_(Vs)
        if self.has_bias:
            self.fc_V.bias.data.copy_(fc.bias.detach().data)
        ...
        self.mask_v_times = torch.zeros(
            self.out_features, self.u_out_features, device="cuda"
        )
        self.mask_u_times = torch.zeros(self.u_out_features, device="cuda")
        self.aggr_v_coeff = torch.zeros(
            self.out_features, self.u_out_features, device="cuda"
        )
