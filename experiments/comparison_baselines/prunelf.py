""" *
 *  Implementation for PruneFL
 *  The code in folder ./base is based on the official implementation of the paper:
 *  PruneFL: Model pruning enables efficient federated learning on edge devices
 *  Official implementation: https://github.com/jiangyuang/PruneFL
"""
import torch
...
class PruneFLViT(nn.Module):
    ...
    def forward(self, inp: torch.Tensor):
        if self.transpose:
            return AddmmFunction.apply(
                self.bias, self.weight, self.dense_weight_placeholder, inp.t()
            ).t()
        else:
            return AddmmFunction.apply(
                self.bias, self.weight, self.dense_weight_placeholder, inp
            )
    ...
    def __repr__(self):
        return "SparseLinear(in_features={}, out_features={}, bias={}, transpose = {})".format(
            self.in_features, self.out_features, self.bias is not None, self.transpose
        )
...
class DenseLinear(nn.Module):
    ...
    def __init__(
        self, in_features, out_features, use_bias=True, use_mask=True, **kwargs
    ):
    ...
    def prune_by_rank(self, rank):
        if rank == 0:
            return
        weight_val = self.weight[self.mask == 1.0]
    ...
    def retain_by_rank(self, rank):
        weights_val = self.weight[self.mask == 1.0]
    ...
    def to_sparse(self, transpose=False) -> SparseLinear:
        ...
        sparse_linear = SparseLinear(
            (self.weight * self.mask).to_sparse(), sparse_bias, self.mask
        )
        if transpose:
